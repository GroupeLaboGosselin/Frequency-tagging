%% SSVEP features analysis : Emotion and Gender discrimination task for the Bubbles x SSVEP project.
%
% Simon Faghel-Soubeyrand,2016
...
{'RG','SSVEP_BubblesGender_Rose_16aout.cnt','Gender','RG',6}
...
{'MTQ','Mariethe_SSVEP_Bubbles_Gender_22aout.cnt','Gender','MTQ',6} %?	Problème avec l'électrode p3 durant le dernier essai du deuxième block. L'impédance est revenue à 256. En posant le casque on a eu des difficultés avec les électrodes PO3 et POZ.
{'MTQ','Mariethe_SSVEP_Bubbles_Emotion_22aout.cnt','Emotions','MTQ',6} % FACE TAG

{'ER','EmilieR_SSVEP_Bubbles_Gender_21aout.cnt','Gender','ER',6} % N.B. Une image à figée durant le deuxième block de la tâche gender et l'expérience a boguée. On a été obliger de recommencer le block au complet.
...
{'SVT','SSVEP_Bubbles_Gender_SVT_31_aout_2018.cnt','Gender','SVT',6} % Aucun problème a priori
{'SVT','SVT_SSVEP_Bubbles_Emo_31aout.cnt','Emotions','SVT',6} % Aucun problème a priori

{'GB','GuillaumeB-SSVEP_Bubbles_Gender_29aout.cnt','Gender','GB',6} % problème électricité + bug voir .docx
% {'GB','GuillaumeB_SSVEP_Bubbles_Gender1_29aout.cnt','Gender','GB',6} % problème électricité + bug voir .docx
% {'GB','GuillaumeB_SSVEP_Bubbles_Gender2Bloc4et5_29aout.cnt','Gender','GB',6} % problème électricité + bug voir .docx
...
for subject=19
    ...
    cfg.dataFile            = ['/Volumes/Seagate Thin Plus/DATA_EEG/Data_SSVEPBubbles/SSVEP/', cfg.dataset];
    % % % % % % % For this here, you should put your path ! ! ! ! ! !
    cfg.hdr               = ft_read_header(cfg.dataFile);
    cfg.trialdef.eventtype  = 'trigger';
    
    cfg.demean              = 'yes';
    cfg.trialdef.pre        = [.2 0];
    cfg.trialdef.post       = [.25 1];
...
%   cfg.baselinewindow      = [-.400 -.100];   % baseline?
    ...
    cfg.refchannel    = 'all';     %{'M1','M2','Fp1'}%;'all';'all'
...
%     raw_data = ft_appenddata(cfg,raw_data_1,raw_data_2,raw_data_3)
    ...
    %     HERE, WE SAVE DATA
    % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
        eval(sprintf('save(''Trialed_avg_data_SSVEP_%s_%s'',''raw_data'')',Subjects{subject}{1},Subjects{subject}{3}))
end
%% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
...
%     
    ...
    cfg.dataFile            = [/Volumes/Seagate Thin Plus/DATA_EEG/Data_SSVEPBubbles/SSVEP/', cfg.dataset];
    cfg.hdr                 = ft_read_header(cfg.dataFile);
    electrodes = ft_read_sens('/Volumes/Seagate Thin Plus/DATA_EEG/Data_SSVEPBubbles/SSVEP/standard_waveguard64.elc');
    cfg.elec=electrodes;
...
%     eval(sprintf('load(''TrialedInter_data_SSVEP_%s_%s'',''raw_data'')',Subjects{subject}{1},Subjects{subject}{3}))
    ...
    for block=1:Subjects{subject}{5}
        behav{block}=eval(sprintf('load(''/Volumes/Seagate Thin Plus/DATA_EEG/Data_SSVEPBubbles/SSVEP/DATA_SSVEPBubbles/DataBehav/%s_SSVEP%s_%d.mat'')',Subjects{subject}{4},Subjects{subject}{3},block));
...
cd '/Volumes/Seagate Thin Plus/DATA_EEG/Data_SSVEPBubbles/SSVEP/SSVEP_Bubbles_DATA/ProcessedData'
tasks={'Gender','Emotions'};
for subject=18:19

%     eval(sprintf('load(''/Volumes/Seagate Thin Plus/DATA_EEG/Data_SSVEPBubbles/SSVEP/Trialed70_avg_data_SSVEP_%s_%s'',''raw_data'')',Subjects{subject}{1},Subjects{subject}{3}))
...
%     eval(sprintf('save(''frequencySpctrm2_Response_SSVEP_%s_%d.mat'',''Freq_sub'',''freqcfg'')',tasks{mod(subject,2)+1},subject))
    eval(sprintf('save(''frequencySpctrm2_SSVEP_%s_%d.mat'',''Freq_sub'',''freqcfg'')',tasks{mod(subject,2)+1},subject))
     waitbar(subject/15)
end
delete(h)
...
% % - - - -- --- - -frequency band - - - - - - - - - - - -% % % 
freqRange=range(freq{subject}.freq);
freqRes=freqRange/size(FeatFreq_indx,2);

% Alpha
lowFreqBand=8;
highFreqBand=12.5;
rangeBand=round((highFreqBand-lowFreqBand)/freqRes);

FeatFreq_indx = abs(bsxfun(@minus,lowFreqBand,freq{subject}.freq));
[~,index_freqToFeatures] = min(FeatFreq_indx(:,1:size(FeatFreq_indx,2)));

ThoseIndexes=index_freqToFeatures:(index_freqToFeatures+rangeBand);

ThoseIndexes(find(ismember(ThoseIndexes,ALL_GOODindexes)))=[];
AlphaIndexes=ThoseIndexes;
...
ThoseIndexes=index_freqToFeatures:(index_freqToFeatures+rangeBand);
ThoseIndexes(find(ismember(ThoseIndexes,ALL_GOODindexes)))=[];
BetaIndexes=ThoseIndexes(1:end-1);
...
for subject=Subj
  ...
  eval(sprintf('load(''frequencySpctrm2_SSVEP_%s_%d.mat'',''Freq_sub'',''freqcfg'')',tasks{mod(subject,2)+1},subject))
...
figure,
 for perm=1:6 
  ...
  subplot(3,2,perm),plot(freq{Subj(1)}.freq,squeeze(nanmean(PERM_powspctrm(Subjs_cond,perm,electrodesAll,:)))),axis([ 2 20 0 .2])
%  subplot(3,2,perm),plot(freq{Subj(1)}.freq,squeeze(nanmean(nanmean(PERM_powspctrm(:,perm,electrodesAll,:)),3)),'-k'),axis([ 2 20 0 .2]),title(sprintf('permutation %d',perm))
         hold on
         bar(Perm(perm,1) ,.005,.3,'r'),
         hold on
         bar(Perm(perm,2),.005,.3,'b')
         hold on
         bar(Perm(perm,3),.005,.3,'g')
         if perm==1
...
%             hz_intermod(11)=abs(2*Perm(perm,1)+Perm(perm,2));
        ...
        for intermodul=1:12
            ...
            whichFreqNoiseNeg=nanmean(squeeze((PERM_powspctrm(sub,perm,:,whichNegIdx))))';
            whichFreqNoisePos=nanmean(squeeze((PERM_powspctrm(sub,perm,:,whichPosIdx))))';
            %             SNR_temp= snr(whichFreqSignal(elec),nanmean([whichFreqNoiseNeg; whichFreqNoisePos]));
            %             if index_freqToFeatures>interval
            SNR_temp=whichFreqSignal./(nanmean(([whichFreqNoiseNeg; whichFreqNoisePos])));
            %
            SNR_tempZ=whichFreqSignal-(nanmean(([whichFreqNoiseNeg; whichFreqNoisePos])));
            SNR_tempZ=SNR_tempZ./(std([whichFreqNoiseNeg(:); whichFreqNoisePos(:)]));
            ...
            %
            SNR_IMeye{intermodul}(sub,perm,:)=SNR_tempZ;%SNR_temp;%whichFreqSignal;%
...
%%  permutations for bootstrap (noise) SNR
clear SNR_noise_allIter SNR_noise_allIter SNR_noise
for iter=1:500
    subCounter=0;
    for sub=1:subCounter
        clear WhichPerm whichPerm_boot
        WhichPerm=TrialTagg{sub};
        for ii=1:6,
            if numel(TrialTagg{sub}(TrialTagg{sub}==ii))<2
                WhichPerm=TrialTagg{sub}(TrialTagg{sub}~=ii);
            end
        end
        subCounter=subCounter+1;
        RandIdx=randperm(numel(WhichPerm));
        whichPerm_boot=WhichPerm(RandIdx);
        for perm=1:6
            %     hz_intermod=(intermod(tagg',[0.5 50]))
            ...
            for intermodul=1:9
                ...
                whichFreqSignal=squeeze((PERM_powspctrm(subCounter,whichPerm_boot(ceil(10*rand(1))),:,index_freqToFeatures)))';
                
                whichRandIdx=randperm(size(PERM_powspctrm,4)-interval,1);
                whichFreqNoise=squeeze((PERM_powspctrm(subCounter,perm,:,noiseIndexes(whichRandIdx))))';
                if whichRandIdx>interval
                    whichNegIdx=(whichRandIdx-interval):(whichRandIdx-5);
                end
                whichPosIdx=(whichRandIdx+5):(whichRandIdx+interval);
                if ismember(whichNegIdx,ALL_GOODindexes)
                    whichNegIdx(find(ismember(whichNegIdx,ALL_GOODindexes)))=whichNegIdx(find(ismember(whichNegIdx,ALL_GOODindexes)))-1;
                end
                if ismember(whichPosIdx,ALL_GOODindexes)
                ...
                whichFreqNoiseNeg=nanmean(squeeze((PERM_powspctrm(subCounter,whichPerm_boot(intermodul),:,whichNegIdx))))';
                whichFreqNoisePos=nanmean(squeeze((PERM_powspctrm(subCounter,whichPerm_boot(intermodul),:,whichPosIdx))))';
                %             SNR_temp= snr(whichFreqSignal(elec),nanmean([whichFreqNoiseNeg; whichFreqNoisePos]));
                %             if index_freqToFeatures>interval
                if (nanmean(([whichFreqNoiseNeg; whichFreqNoisePos])))==0 || std([whichFreqNoiseNeg(:); whichFreqNoisePos(:)])==0
                    sub
                    %     break;
                end
                
                SNR_temp_boot=whichFreqNoise./(nanmean(([whichFreqNoiseNeg; whichFreqNoisePos])));
                %
                SNR_tempZ=whichFreqNoise-(nanmean(([whichFreqNoiseNeg; whichFreqNoisePos])));
                SNR_tempZ=SNR_tempZ./(std([whichFreqNoiseNeg(:); whichFreqNoisePos(:)]));
                
                % %
                SNR_tempZ_noise=whichFreqNoise-(nanmean(([whichFreqNoiseNeg; whichFreqNoisePos])));
                SNR_tempZ_noise=SNR_tempZ_noise./(nanstd([whichFreqNoiseNeg(:); whichFreqNoisePos(:)]));
                
                SNR_temp_noise=whichFreqNoise./(nanmean(([whichFreqNoiseNeg; whichFreqNoisePos])));
                
                SNR_noise{intermodul}(subCounter,perm,:)=SNR_temp_boot;%whichFreqSignal;%SNR_temp;%whichFreqSignal;%SNR_temp;
                SNR_noise_allIter{intermodul}(iter,subCounter,perm,:)=SNR_temp_boot;
            end
end
end
end
[MUHAT,SIGMAHAT,MUCI,SIGMACI] = normfit(SNR_noise{1}(~isnan(SNR_noise{1})),.01);

pthresh=.0001;
[MUHAT,SIGMAHAT,MUCI,SIGMACI] = normfit(SNR_noise_allIter{intermodul}(~isnan(SNR_noise_allIter{intermodul})),.01)
clear dataz critZ
for intermodul =1:9
for electrodes=1:64,
    
        dataz=squeeze(nanmean(SNR_noise_allIter{intermodul}(:,:,:,electrodes),3));
        dataz=dataz(:);
        critZ{intermodul}(electrodes,:)=prctile(dataz,[(pthresh/2)*100,(1-(pthresh/2))*100]);
        %         ps(feat)=Feat_all{object}(feat)<critZ(feat,1)|Feat_all{object}(feat)>critZ(feat,2);
        %         else
        %         ps(feat)=0;
        %         end
...
% ProbFunction=normpdf(SNR_noise{1}(~isnan(SNR_noise{1})),MUHAT,SIGMAHAT);
% thatValues = [-2,-1,0,1,2];

% y = cdf('normal',ProbFunction,thatValues)

% LSL = -3; USL = 3;                     % Process specifications
% capable = @(x)(USL-LSL)./(std(x));  % Process capability
% ci = bootci(1,capable,SNR_noise{1}(~isnan(SNR_noise{1})));            % BCa confidence interva
COND_noise{CONDITION}=SNR_noise;
% COND_IM{CONDITION_attend}=SNR_IMeye;%SNR_noise;%SNR_IMeye;
%%
% load ElectrodesMatrices.mat
...
% which(which==0)=[];
% ANOVA_ELEC1=permute(repmat(which,2,15,1),[1,3,2]);

figure, bar([nanmean(COND_all(ANOVA_COND==1)) nanmean(COND_all(ANOVA_COND==2)) nanmean(COND_all(ANOVA_COND==3))])
sigstar({[1,2]},[p(1)])
% sigstar({[1,2]},[p])


% figure, plot(nanmean(squeeze(nanmean(COND_all)),2))
% figure, plot((squeeze(nanmean(COND_all))))

%  P=anova_rm({[squeeze((COND_all(1,:,:)))] [squeeze((COND_all(2,:,:)))]})
...
electrodesLeft=24;%find(elec_postLeft_ind);
electrodesRight=54;%find(elec_postRight_ind);
electrodesMid=31;%find(elec_postMid_ind);
...
anovaCond_test1=zeros(size(test1));
temp_sub=[2:2:length(whichSubs)];
anovaCond_test1(:,temp_sub,:)=ones(size(test1(1:3,temp_sub,:)));

anovaFeat_test1(1,:,:)=ones(size(test1(1,:,:))).*1;
anovaFeat_test1(2,:,:)=ones(size(test1(1,:,:))).*1;
anovaFeat_test1(3,:,:)=ones(size(test1(1,:,:))).*2;
...
whichelecs=find(elec_post_ind);

temp_sub=[1:2:length(whichSubs)-1];
...
figure, shadedErrorBar(1:size(ys,2),ys,errorstd,'o--r',1),title('Feature signal'),legend({'left-eye signal','right-eye signal','mouth signal'})
temp_sub=[2:2:length(whichSubs)];
ys=[nanmean(nanmean(COND_all_mean(1,temp_sub,whichelecs))) nanmean(nanmean(COND_all_mean(2,temp_sub,whichelecs))) nanmean(nanmean(COND_all_mean(3,temp_sub,whichelecs)))]; errorstd=nanstd([(nanmean(COND_all_mean(1,temp_sub,whichelecs),3)); (nanmean(COND_all_mean(2,temp_sub,whichelecs),3)); (nanmean(COND_all_mean(3,temp_sub,whichelecs),3))],0,2)/sqrt(length(temp_sub));
hold on
shadedErrorBar(1:size(ys,2),ys,errorstd,'o--k',1),title(sprintf('Feature signal per task (red:Gender,p_interaction=%.4f)',P(6))),legend({'gender task','emotion task'})%,legend({'left-eye signal','right-eye signal','mouth signal'})
if length(perm)>2
...
% 
hdmfile = '/Users/SimonFaghel-Soubeyrand/Documents/MATLAB/fieldtrip-master/template/headmodel/standard_bem.mat';
...
if COND_task==1
    subjects_condition=[2:2:18]-1;
%     subjects_condition=[1];
elseif COND_task==2
    subjects_condition=[2:2:18];
...
for whichInter=1:3%[1 2 3]+6
    ...
    counter=counter+1;
    Features_signal{COND_task}{whichInter}=squeeze(nanmean(COND_IM{COND}{whichInter}(subjects_condition,perm,:,:),2));
    switch mod(whichInter,3)
        case 0
            count{3}=count{3}+1;
            Features_signal{COND_task+2}{3}(count{3},:,:)=squeeze(nanmean(COND_IM{COND}{whichInter}(subjects_condition,perm,:,:),2));
        case 1
            count{2}=count{2}+1;
            Features_signal{COND_task+2}{2}(count{2},:,:)=squeeze(nanmean(COND_IM{COND}{whichInter}(subjects_condition,perm,:,:),2));
        case 2
            count{1}=count{1}+1;
            Features_signal{COND_task+2}{1}(count{1},:,:)=squeeze(nanmean(COND_IM{COND}{whichInter}(subjects_condition,perm,:,:),2));
    end
    subj_2=[2:2:18]
    subj_2(2)=[];
       vector{COND_task}{whichInter}=(squeeze(nanmean(nanmean(COND_IM{COND}{whichInter}(subjects_condition,perm,:,:)),2))-squeeze(nanmean(nanmean(COND_IM{COND}{whichInter}(subj_2,perm,:,:)),2)))/sqrt(2);%-squeeze(nanmean((COND_IM{COND}{whichInter+3}(subj,perm,:,:)),2));
...
%        show map
       subplot(1,3,counter)
       ft_topoplotER(plotcfg, freq_bog{1}),colormap(plasma)
        set(gca, 'CLim', [-3 3]);
...
%      end
    ...
    subplot(2,3,counter),title(Feats_labels{mod(whichInter,3)+1})
...
electrodes = ft_read_sens('/Volumes/Seagate Thin Plus/DATA_EEG/standard_waveguard64.elc');
...
Stats.label=freq{89}.label%freq{9}.label;
%
...
%     waitbar((ii/nbElec), h, sprintf('ttest : %3.2f %% complété', (ii/14)));
% end
delete(h)
pmap1=(pmap1);
tmap1=(tmap1);
...
for subject=2:15
    ...
    eval(sprintf('load(''/Volumes/Seagate Thin Plus/DATA_EEG/Data_SSVEPBubbles/SSVEP/TrialedResponse_avg_data_SSVEP_%s_%s'',''raw_data'')',Subjects{subject}{1},Subjects{subject}{3}))
%     freqcfg.trials='all';
...
%%
%% INTERMOD TOTAL
Subj_cond1=[8]; %cond 1 only
Subj_cond2=[9];
subject=Subj_cond2(1);
%   load 'Intermodulary_frequenciesSubjects.mat'
load 'Intermodulary_frequenciesSubjectsCOND2.mat'





MDiff.powspctrm=zeros(1,size(freq{subject}{1}.powspctrm,2))
clear InterFeature_topo Inter_topo Inter_difference


load FREDSSVEPWINKCOND1_blck1_SSVEP_0_1.mat
trial_features=cell2mat(data{1}.freq);
hz_intermod=round(intermod([trial_features]',[0.5 20]),1)

load('quickcap64.mat') % load 2D layout of electrodes
posterior_elec = {'T7','T8','P7','P3','Pz', 'P4', 'P8', 'POz', 'O1', 'Oz', 'O2', 'P5', 'P1', 'P2', 'P6', 'PO5', 'PO3', 'PO4', 'PO6', 'TP7', 'TP8','PO7','PO8'};
plotcfg.channel=posterior_elec;%cfg.hdr.label(1:64)%freqcfg.channel;
plotcfg.interpolatenan ='yes';
plotcfg.interplimits = 'head';
plotcfg.interpolation ='nearest';%'nearest'
plotcfg.colorbar='East';
plotcfg.layout=lay;
% plotcfg.foilim=[;
plotcfg.parameter='powspctrm';
...
for subject=Subj_cond2;
    ...
    for ii=1:length(hz_intermod)
        InterFeature_topo{subject}{ii}=freq{subject}{1};
        InterFeature_topo{subject}{ii}.powspctrm=zeros(1,size(freq{subject}{1}.powspctrm,2));
    end
    
    blockCounter=1;
    trialBlckCounter=0;
    
    eval(sprintf('load(''%s_blck%d_SSVEP_0_1.mat'') ',Subjects{subject}{3} ,blockCounter))
    for trial=1:size(freq{subject}{1}.powspctrm,1)
        trialBlckCounter=trialBlckCounter+1;
        ...
        trial_features=round(cell2mat(data{trialBlckCounter}.freq),1);
        
        hz_intermod=round(intermod([trial_features]',[0.5 20]),1); % three first intermod are betwee left and right eye, three after are for left eye and mouth, and three lasts are for mouth and right eye
        for frequency=1:length(hz_intermod)
            C = abs(bsxfun(@minus,hz_intermod,mean(freq{subject}{frequency}.freq)));
            [~,index_freqToFeatures] = min(C(:,1:size(C,2)))
            if ~any(index_freqToFeatures)
                C = abs(bsxfun(@minus,harmon_feat',mean(freq{subject}{frequency}.freq)));
                [~,index_freqToFeatures] = min(C(:,1:size(C,2)))
                harmon=harmon+1; %  this/3 should be equal to number of trials
            end
            InterFeature_topo{subject}{index_freqToFeatures}.powspctrm(:)=InterFeature_topo{subject}{index_freqToFeatures}.powspctrm(:)+mean(freq{subject}{frequency}.powspctrm(trial,:,:),3)';
        end
    end
    
    interLabel={'Left && Right', 'Left && Mouth', 'Right && Mouth','All','AVG','Blink non-linearity'};
    
    for ii=1:length(hz_intermod)
        Inter_topo{subject}{ii}=freq{subject}{1};
        Inter_topo{subject}{ii}.powspctrm=zeros(1,size(freq{subject}{1}.powspctrm,2));
        Inter_topo{subject}{ii}.freq=1;
        Inter_difference{subject}{1}=freq{subject}{1};
        Inter_difference{subject}{1}.powspctrm=zeros(1,size(freq{subject}{1}.powspctrm,2));
        Inter_difference{subject}{1}.freq=1;
    end
    
    
    Inter_topo{subject}{1}.powspctrm(:)=(InterFeature_topo{subject}{1}.powspctrm(:)+InterFeature_topo{subject}{2}.powspctrm(:))/(2*size(freq{subject}{1}.powspctrm,1));
    
    Inter_topo{subject}{2}.powspctrm(:)=(InterFeature_topo{subject}{3}.powspctrm(:)+InterFeature_topo{subject}{4}.powspctrm(:))/(2*size(freq{subject}{1}.powspctrm,1));
    
    Inter_topo{subject}{3}.powspctrm(:)=(InterFeature_topo{subject}{5}.powspctrm(:)+InterFeature_topo{subject}{6}.powspctrm(:))/(2*size(freq{subject}{1}.powspctrm,1));
    
    Inter_topo{subject}{4}.powspctrm(:)=InterFeature_topo{subject}{7}.powspctrm(:);%+InterFeature_topo{subject}{6}.powspctrm(:))/(2*size(freq{subject}{1}.powspctrm,1));
    
    for inter_Feature=1:4, Inter_topo{subject}{5}.powspctrm(:)=Inter_topo{subject}{5}.powspctrm(:)+InterFeature_topo{subject}{inter_Feature}.powspctrm(:);  end
    
    Inter_topo{subject}{5}.powspctrm(:)=Inter_topo{subject}{5}.powspctrm(:)/inter_Feature;
    
    for inter_Feature=1:5, figure,ft_topoplotER(plotcfg, Inter_topo{subject}{inter_Feature}), title(sprintf('%s',interLabel{inter_Feature})); end
    
    
    Inter_difference{subject}{1}.powspctrm(:)=Inter_topo{subject}{1}.powspctrm(:)-(Inter_topo{subject}{2}.powspctrm(:)+Inter_topo{subject}{3}.powspctrm(:))/2;%mean([Inter_topo{subject}{2}.powspctrm(:); Inter_topo{subject}{3}.powspctrm(:)]);
    % figure,ft_topoplotER(plotcfg, Inter_difference{subject}{1}), title(sprintf('%s',interLabel{6}))
    
    MDiff.powspctrm(:)=MDiff.powspctrm(:)+Inter_difference{subject}{1}.powspctrm(:);
end

MDiff.powspctrm=MDiff.powspctrm/length(Subj_cond1);
All_cfgDiff=Inter_difference{subject}{1};
All_cfgDiff.powspctrm=MDiff.powspctrm;
